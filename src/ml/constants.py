...
# 距離バケット定義（厳密なグループ分け）
DISTANCE_BUCKETS: Final[dict] = {
    'sprint': [1000, 1200],           # 短距離スプリント
    'short': [1400],                   # 7ハロン
    'mile': [1600, 1800],              # マイル〜9ハロン
    'intermediate': [2000, 2200],      # 中距離（10-11ハロン）
    'long': [2400, 2500],              # クラシックディスタンス
    'extended': [2600, 3000, 3200, 3400, 3600]  # 長距離（ステイヤー）
}
"""距離バケット定義
- 理由: ±200mの曖昧な許容ではなく、競馬の距離カテゴリに基づく厳密な分類
- スプリンターとマイラーは別適性、混同すると予測精度が低下
- 各バケット内は±50mの許容を持つ
"""

# レースクラス階層定義
RACE_CLASS_HIERARCHY: Final[dict] = {
    'G1': 6, 'G2': 5, 'G3': 4,
    'OP': 3, 'Listed': 3,
    '3勝': 2, '2勝': 1, '1勝': 0,
    '未勝利': -1, '新馬': -2
